...
export function InheritanceCard() {
  ...
  useEffect(() => {
    ...
    const update = () => {
      const deadline = (Number(state.lastPingTime) + Number(state.timeoutDuration)) * 1000;
  ...
  const canExecute = isConnected && timeLeftMs <= 0 && userAddress?.toLowerCase() === state.heir.toLowerCase();
